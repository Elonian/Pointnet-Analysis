...
def step(points, pc_labels, class_labels, model):
    ...
    loss = loss + get_orthogonal_loss(feat_trans)
...
def train_step(points, pc_labels, class_labels, model, optimizer, train_acc_metric, scaler=None, use_amp=False):
    ...
    train_batch_acc = train_acc_metric(preds, pc_labels.to(preds.device))
...
def validation_step(
    ...
    pc_labels = pc_labels.to(preds.device)
...
def main(args):
    ...
    args.amp = args.amp and device != "cpu"
    ...
    for epoch in range(args.epochs):
        ...
        for points, pc_labels, class_labels in pbar:
            ...
            train_epoch_loss.append(train_batch_loss.detach())
        ...
        with torch.no_grad():
            ...
            for points, pc_labels, class_labels in val_dl:
                ...
                val_epoch_loss.append(val_batch_loss.detach())
